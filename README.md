...
**`목록 새로고침`** 버튼을 누르면 됩니다. `plugin_list.txt`를 강제로 다시 조회한 뒤
카드 목록을 새로고침합니다 — 이미 설치된 플러그인의 상태(설치 여부·버전·활성화 여부)에는
전혀 영향을 주지 않고, 순수하게 "어떤 카드를 보여줄지" 목록만 갱신합니다.
...
- **깊이 0** — 압축 최상위에 `{id}.py`, `VERSION` 등이 바로 있는 경우
- **깊이 1** — GitHub에서 받은 zip처럼 `저장소이름-브랜치/` 폴더 하나로 감싸진 경우
- **깊이 2 이상** — `outer/inner/plugin/` 처럼 여러 겹 중첩된 경우

탐색 방식(`_find_plugin_root_dir`)은 압축을 해제한 트리를 순회하면서 "플러그인처럼
보이는 폴더"(`VERSION` 파일이 있거나, `id = "..."`를 선언한 `.py` 파일이 있는 폴더)를
찾고, 그 폴더를 실제 설치 루트로 사용합니다. 못 찾았지만 하위 폴더가 정확히 1개뿐이면
그 폴더를 사용합니다(흔한 "저장소 하나로 감싸진" 패턴 처리).

플러그인 ID(설치될 폴더 이름)는 다음 순서로 결정됩니다.

1. `VERSION` 파일의 `id`/`plugin_id` 값
2. `.py` 파일의 `id = "..."` 선언 — **AST로만** 읽으며 코드를 import/exec 하지 않습니다
3. 찾아낸 플러그인 루트 폴더의 이름
4. 위 셋 다 실패하면 업로드한 zip 파일명(영문/숫자 외 문자는 `_`로 치환)

Git URL 설치와 동일하게 Zip Slip(경로 이탈) 방지 검증을 거치며, `.git`, `__pycache__`,
`__MACOSX`, `.DS_Store` 등 부산물은 복사에서 제외됩니다. `plugin_board` 자기 자신을
덮어쓰는 zip은 거부합니다.
...
- **Zip 업로드도 동일한 경계를 공유**: `_extract_zip_safe`로 Zip Slip을 막고, 찾아낸 플러그인
  루트 폴더 이름은 `_validate_plugin_id`로 검증한 뒤에만 `plugins/metadata/`에 씁니다.
  `plugin_board` 자기 자신을 덮어쓰는 zip은 명시적으로 거부합니다.

## 문제 해결

### Zip 설치 시 "설치 요청 중 통신 오류가 발생했습니다"

zip을 base64로 인코딩해 JSON 본문에 담아 `/api/media/books/0/apply-metadata`로
보내는 방식이라, 원본 파일 크기가 어느 정도만 커도(대략 1MB 안팎) 서버 앞단 프록시
(nginx 등)의 기본 요청 크기 제한(`client_max_body_size`, 흔히 기본값 1MB)에 걸려
`413 Request Entity Too Large` 같은 HTML 오류 페이지가 돌아올 수 있습니다.

- **v2.10.0 이전**: 이 오류 페이지를 JSON으로 파싱하려다 실패해 원인을 알 수 없는
  "통신 오류" 메시지로 뭉뚱그려졌습니다.
- **v2.11.0부터**: 서버 응답이 JSON이 아니면 HTTP 상태 코드와 응답 본문 일부를 그대로
  토스트에 보여줍니다. `HTTP 413`이 보이면 요청 크기 제한 문제이니, 서버 관리자가
  `client_max_body_size`(nginx) 등 프록시 설정을 올리거나, 대신 **Git 저장소 URL
  설치**(§6)를 사용하세요 — 그 경로는 zip을 브라우저→서버로 올리지 않고 서버가 직접
  GitHub에서 내려받으므로 이 제한과 무관합니다.
- 업로드 전에도 파일이 1MB를 넘으면 안내 토스트가 먼저 표시됩니다.

### Zip 설치 시 "HTTP 500 ... openresty" 응답

413과는 다른 증상입니다. `plugin_board.py`가 오류를 만들어서 응답한 게 아니라,
**프록시(openresty/nginx)가 백엔드와의 연결을 못 받았을 때 자체적으로 띄우는 기본
오류 페이지**입니다(응답 본문에 Python 트레이스백이나 JSON이 전혀 없고, 순수
"500 Internal Server Error ... openresty" HTML만 있다면 이 케이스입니다). 주로:

- 파일이 커서 base64 인코딩·전송·서버 처리 시간이 프록시/워커의 타임아웃을 넘긴 경우
- 처리 도중 백엔드 워커(Gunicorn 등)가 다운되거나 재시작된 경우

**v2.12.0부터 다음과 같이 대응했습니다.**

- **하드 차단**: zip이 8MB를 넘으면 업로드 시도 자체를 막고 Git 저장소 URL 설치로
  안내합니다(1~8MB는 경고만 하고 계속 진행).
- **클라이언트 타임아웃**: `callPluginBoardAction`에 60초 타임아웃을 추가했습니다.
  응답이 그 안에 안 오면 요청을 중단하고, "서버가 아직 처리 중이거나 타임아웃/충돌한
  것 같다"는 명확한 안내와 함께 잠시 후 새로고침해서 실제 설치 여부를 확인하라고
  안내합니다.
- **응답 진단 강화**: `HTTP 500` + 본문에 `openresty`/`nginx` 흔적은 있는데 Python
  트레이스백은 없는 패턴을 감지하면, "이건 plugin_board 코드 오류가 아니라 백엔드가
  타임아웃/다운됐을 가능성이 높다"고 구체적으로 안내합니다.
- **서버 쪽 최종 안전망**: `apply()` 전체를 감싸는 예외 처리기를 추가해, 저희가
  예상 못 한 예외가 발생해도 항상 `(False, 메시지)`로 안전하게 반환하도록 했습니다
  (다만 openresty 레벨의 연결 끊김/타임아웃은 애초에 plugin_board 코드에 도달하기
  전이나 도중에 일어나는 문제라 이 안전망 밖의 일일 수 있습니다).

이 오류가 반복되면 서버의 Gunicorn/Flask 로그(요청 시각 기준)를 확인해 실제
트레이스백이나 워커 타임아웃/OOM 로그가 있는지 봐주세요. 그 사이에는 Git 저장소
URL 설치를 쓰시는 게 가장 확실합니다.

**바이트 크기는 작은데(1MB 미만) 500이 나는 경우** — 원본 zip이 작아도 내부에 파일
"개수"가 아주 많으면(전형적으로 `.git` 폴더를 실수로 함께 압축한 경우, `.git/objects`
안에 작은 파일이 수천 개씩 있는 게 흔합니다) 압축 해제·경로 검증·플러그인 루트 탐색에
쓰는 자원이 파일 개수에 비례해 늘어나 타임아웃/다운으로 이어질 수 있습니다.

**v2.14.0부터** `_extract_zip_safe`에 두 가지 제한을 추가했습니다.

- **항목 개수 상한**: 3,000개 초과 시 압축 해제 전에 즉시 거부 (`.git` 등 확인 안내 포함)
- **압축 해제 후 총 용량 상한**: 200MB 초과 시 거부 (전형적인 zip bomb 방지)

두 제한 모두 실제로 압축을 풀기 **전에** 확인하므로, 자원을 소모하지 않고 바로 명확한
오류 메시지로 실패합니다. zip을 올리기 전에 `.git`, `.github`, `node_modules`,
`__pycache__` 같은 폴더가 섞여 있지 않은지 먼저 확인해보시는 걸 권장합니다.
...
- Zip 업로드는 `VERSION` 파일 또는 `id = "..."`를 선언한 `.py` 파일 중 하나는 있어야
  플러그인으로 인식합니다. 폴더 이름과 실제 `id`가 다르면 `id` 값이 우선합니다.
