...
## ZIP 파일 업로드 설치 (v2.35.0)

`madnite1/plugin_manager`의 실제 `_install_from_zip` 구현을 그대로 참고해
재구현했습니다. Git 저장소 URL 설치와 나란히, 화면 상단에 "ZIP 파일 업로드 설치"
패널이 표시됩니다(관리자만 — §관리자 전용 접근 제어 참고).

**동작 방식**

1. 업로드된 zip을 base64로 받아, 임시 폴더에 안전하게 압축 해제합니다
   (Zip Slip 차단 + 항목 개수/총 용량 상한 — §5의 codeload zip 해제와 동일한
   `_extract_zip_safe`를 그대로 재사용).
2. 압축 해제된 폴더 안에서(중첩 폴더여도) 실제 플러그인 루트를 지능 탐색하고,
   `VERSION`/코드 내 `id=` 속성/폴더명/zip 파일명 순으로 플러그인 ID를 결정합니다.
...
5. **`update_manifest.raw_base_url`이 GitHub 루트(또는 Gitea) 저장소를 정확히
   가리키면**, 설치 방식(zip)과 무관하게 §2-2의 `github.txt` 레지스트리에
