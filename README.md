...
**`plugin_board` 자기 자신은 목록에 없어도 항상 카드로 표시됩니다.** 개발 중이라
`plugin_list.txt`에 아직 등록하지 않은 버전이라도, `get_dashboard_data()`가 원격
목록을 가져온 뒤 그 안에 `plugin_board`가 없으면 `SELF_REPO_URL`
(`https://github.com/yume-script/plugin_board`)을 자동으로 추가합니다. 그 결과 자기
자신도 다른 카드와 완전히 동일하게 설명·최신 버전·업데이트 확인·`업데이트` 버튼을
그대로 씁니다 — 코어의 별도 자동 업데이트 화면(환경설정 > 플러그인 설정의 샘플 업데이트
버튼)에 기대지 않고, 이 화면 안에서 스스로를 관리할 수 있습니다. 단, 삭제·비활성화는
`_delete_plugin`/`_toggle_plugin_enabled`가 여전히 거부하며, 프론트엔드에서도 자기
카드에는 그 두 버튼 자체를 표시하지 않습니다(눌러도 거부될 걸 미리 숨김).
