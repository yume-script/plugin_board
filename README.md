...
**`목록 새로고침`** 버튼을 누르면 됩니다. `plugin_list.txt`, GitHub Topics 발견
결과(§2-1), **그리고 설명·최신 버전 캐시(`_DESC_CACHE`/`_VERSION_CACHE`)까지 모두
강제로 다시 조회**합니다(v2.34.1부터 — 이전 버전은 목록/토픽 캐시만 비우고
버전 캐시는 그대로 둬서, 버튼을 눌러도 "설치된 버전 vs 최신 버전" 비교 결과가
최대 1시간 동안 옛날 값으로 남아있는 문제가 있었습니다). 설치 여부·설치된
버전·활성화 여부는 항상 서버 파일시스템에서 즉시 읽어오므로(§3) 이 버튼과
무관하게 이미 최신 상태이며, 이 버튼이 갱신하는 건 "GitHub/Gitea에 최신 버전이
있는지" 쪽입니다.
...
## plugin_list.txt 조회 실패가 전체 화면을 막지 않도록 (v2.39.0)

**증상**: `plugin_list.txt` 조회가 실패하면(네트워크 문제, 서버 재시작 직후라
캐시가 전혀 없는 상태 등) 화면 전체가 "플러그인 목록을 불러오지 못했습니다"
오류로 바뀌면서 **이미 설치된 플러그인조차 하나도 안 보이는** 문제가 있었습니다.
설치 여부는 GitHub 접속과 전혀 무관하게 서버 파일시스템만으로 확인 가능한데도,
큐레이션 목록(plugin_list.txt) 조회 실패 하나가 전체를 막고 있었던 것입니다.

**고친 내용**:

- `plugin_list.txt` 조회가 실패해도 더 이상 전체 실패로 처리하지 않습니다.
  `plugin_board` 자기 자신(§SELF_REPO_URL), github.txt 레지스트리(§2-2),
  서버에 실제로 설치된 로컬 플러그인(§4)은 plugin_list.txt와 무관한 별도
  경로로 조회되므로 계속 정상 표시됩니다.
- 대신 응답에 `list_warning` 필드가 함께 내려오고, 화면에는 카드 목록을 가리는
  대신 **카드 위에 작은 노란색 경고 배너**로만 표시됩니다("⚠ plugin_list.txt를
  가져오지 못해 큐레이션 목록이 최신이 아닐 수 있습니다..."). 다음 새로고침이나
  자동 재시도로 조회가 다시 성공하면 배너는 자동으로 사라집니다.
- 원격 조회 타임아웃도 6초 → 10초로 늘려, 서버-GitHub 간 왕복 지연이 약간
  긴 네트워크 환경에서의 일시적 타임아웃 실패를 줄였습니다.
- 실제로 "캐시 전혀 없음 + plugin_list.txt 완전 실패"라는 최악의 시나리오를
  재현해, `plugin_board` 자기 자신과 로컬 설치 플러그인이 정상적으로 카드에
  뜨는 것까지 확인했습니다.
