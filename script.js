...
(function () {
  ...
  // ------------------------------------------------------------------
  // "Zip 파일 업로드 설치" 패널 — 파일을 base64로 읽어 apply({action:"install_zip"})로
  // 전송한다. 플러그인 루트 탐색·ID 식별·정적 소스 검증·2차 로드 검증은 전부
  // 백엔드(_find_plugin_root_dir/_detect_plugin_id/_validate_plugin_source/
  // _verify_plugin_loaded)가 처리한다.
  // ------------------------------------------------------------------
  ...
  function wireZipInstallPanel() {
    ...
    // 프록시 요청 크기 제한(413)이나 처리 시간 초과로 실패하기 쉽다.
